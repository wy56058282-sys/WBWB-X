...
describe('home hero icon navigation', () => {
  it('defines the update ticker motion, pause, and reduced-motion contracts', () => {
    ...
    expect(css).toMatch(
      /\.wbx-update-ticker\s*\{[^}]*height:\s*28px;[^}]*display:\s*grid;[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\);[^}]*border:\s*0;[^}]*transform:\s*translateY\(-10px\);[^}]*background:\s*transparent;/s,
    )
    expect(css).toMatch(
