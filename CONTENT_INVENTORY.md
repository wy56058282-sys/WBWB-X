...
| 内容类型 | 数量 | 说明 |
| --- | ---: | --- |
| 对外 Markdown 页面 | 59 | 不含内部设计与实施记录 |
| 白皮书正文 | 27 章 | 第一篇 10 章、第二篇 11 章、第三篇 4 章、第四篇 2 章 |
| OPC 白皮书 | 6 章 | 趋势、商业模式、政策、WorkBuddy 解法、案例与生态合作 |
...
| OPC 专区 | `/opc/` | OPC 白皮书总览、6 章全文及原版 PDF |
...
docs/.vitepress/opc-sidebar.ts
```

`/wb-x/` 使用 27 章实战白皮书侧边栏；`/opc/` 使用 OPC 总览及 6 章侧边栏。两者共享同一阅读布局、内容样式和响应式规则。社区、投稿和帮助页面不显示白皮书侧边栏。
...
| 编号 | 内容 |
| --- | --- |
| 总览 | WorkBuddy OPC 白皮书：AI 时代一人公司的商业与技术重构 |
...
原版 PDF 位于 `docs/public/opc/workbuddy-opc-whitepaper.pdf`，网页全文与 PDF 保持同一章节结构。
