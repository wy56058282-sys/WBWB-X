...
- 源视觉真值：本任务 Browser Comment 1 的最新标注截图；应用未向文件系统暴露该附件路径，源图像素为 `1280 × 1011`，标注区域约为 `139 × 126px`。
- 设计要求：保留右下角 WorkBuddy 机器人，用紫底白标、带朝下尾巴的圆角方形气泡替换两个版本标签，并链接到星火集官网。
- 桌面实现截图：`/private/tmp/home-sparkx-bubble-half-desktop.png`，浏览器 DOM 视口为 `1422 × 938` CSS 像素。
- 移动实现截图：`/private/tmp/home-sparkx-bubble-half-mobile.png`，浏览器 DOM 视口为 `433 × 938` CSS 像素。
- 状态：浅色主题、首页顶部、无菜单或弹窗打开。
- 浏览器截图后端存在像素密度归一，因此布局判断使用 DOM 实测尺寸，未用截图像素反推 CSS 尺寸。

## 必查视觉表面

- 字体与排版：新入口只使用正式 Logo，不增加文字；页面原有标题、正文和导航排版没有变化。
- 间距与布局：按最新标注将气泡缩小一半；桌面实测约 `52 × 52px`，与机器人间距约 `18px`；移动端实测约 `38 × 38px`，与机器人间距约 `13px`。朝下尾巴同步缩小并位于两者间隙内，没有遮挡机器人或相邻图标。
- 色彩与令牌：气泡使用星火集正式品牌紫 `#6c5ce7` 与白色标志；无阴影，键盘焦点继续使用本站薄荷绿焦点色。
- 图片质量：直接使用现有 `/brand/partners/sparkx.svg` 矢量素材；WorkBuddy 机器人继续使用原 PNG，未裁切、重绘或替换。
- 文案与内容：旧“中国版 v5.3.14”和“国际版 v5.2.7”标签均已移除；新入口的无障碍名称为“访问星火集”。

## 全视图与重点区域对比

- 全视图对比确认 Hero 文案、五张像素图标卡片、WB-X 标志、机器人和下方价值条均保持原位置与层级，仅用户标注区域发生变化。
- 重点区域对比确认两个黑色版本标签被一个紫色圆角气泡取代；气泡尾巴朝向保留的机器人，未新增文字或投影。按最新标注增加 `6px` 上下浮动，最接近机器人时仍保持原始间距。
- 桌面页面 `scrollWidth=1405px < viewport 1422px`；移动页面 `scrollWidth=416px < viewport 433px`，均无页面级横向溢出。

## 交互、响应式与运行检查

- 星火集入口的 `href` 为 `https://www.sparkx.zone/`，`target="_blank"`，`aria-label="访问星火集"`，并包含 `rel="noopener noreferrer"`。
- 浏览器中旧版本标签数量和旧 WorkBuddy 官网链接数量均为 `0`。
- 桌面计算样式为 `animation-name: wbx-sparkx-bubble-float`、`animation-duration: 4.2s`、`animation-iteration-count: infinite`、`box-shadow: none`；稳定布局后两次采样确认纵向位置随动画变化。
- 悬停或键盘聚焦时动画暂停；系统启用“减少动态效果”时动画关闭。
- 应用内浏览器控制台 warning/error 为 `0`。

## Findings 与对比历史

- 初次实现使用桌面 `104px`、移动端 `76px` 气泡，未发现 P0、P1 或 P2 问题。
- 用户追加“缩小一半”标注后，将本体、圆角、边框和尾巴同步缩放为 50%，并重新水平居中；复查桌面 `52px`、移动端 `38px` 均无重叠、溢出或控制台错误。
- 用户追加“增加上下浮动效果”标注后，仅为气泡整体增加 `4.2s` 平滑循环的 `6px` 上浮回落；尾巴随本体同步运动，尺寸、定位和机器人保持不变。
- 源视觉截图与实现截图的浏览器画布尺寸不同，因此不做无依据的像素级位置比较；气泡和机器人使用同页面 DOM 实测尺寸与间距验收。
